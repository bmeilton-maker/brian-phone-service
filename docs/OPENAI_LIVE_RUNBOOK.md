...
OPENAI_LIVE_HANGUP_DELAY_MS=4000             # max wait for the goodbye audio to start (covers one backend round trip)
OPENAI_LIVE_FAREWELL_HANGUP=true             # hang up on goodbye intent, not only on end_call
OPENAI_LIVE_FAREWELL_SILENCE_MS=4000         # callee said goodbye + agent silent this long -> hang up
...
4. Finish the task. It says one goodbye and the line drops about a second after the goodbye finishes (`raw.close_ms`). Say "bye" over its goodbye: no second farewell turn should follow. Say "wait, one more thing" over its goodbye: it should stop, answer, and close again after the next goodbye.
...
1. **goodbye**: wait up to `OPENAI_LIVE_HANGUP_DELAY_MS` for the agent's goodbye audio to start, then until it has been quiet for 700 ms (cap 10 s). A callee interruption with content ("wait", "which Tuesday?") cancels here (`openai_live.close_cancelled`, `raw.close_cancels`), acknowledgements and farewells ("okay", "thanks, bye") do not; after 2 cancels the close is firm. `end_call` while the agent is not speaking and says nothing more hangs up when the wait expires.
2. **done**: callee audio is no longer forwarded to the model (`inputMuted`), so a "bye" back cannot start another farewell turn, and no tool result triggers `response.create` any more. If no `report_outcome` was recorded yet, wait 2 s for one (5 s when a delegation is already in flight) so the structured result is not lost. `raw.goodbye_done_ms`.
3. **playout**: a Twilio `mark` is queued behind the goodbye audio; Twilio echoes it when the callee has actually heard it (cap 3 s). Our socket being quiet is not the same thing.
4. **hangup**: Twilio hangup + local finalize (`raw.close_ms`), then `session.close`. `raw.end_reason` is the backend's `end_call` reason when it came, else `agent_said_goodbye` / `callee_said_goodbye`.
...
2. **Goodbye vs hangup.** See "Hangup on farewell". Check `raw.close_trigger`: `end_call` means the prompt path worked; `agent_farewell` means the live model closed without delegating (outcome then comes from the 2 s post-goodbye window or transcript extraction); `human_farewell` means the agent went silent after the callee's goodbye. If the goodbye is clipped, `raw.close_ms` minus `raw.goodbye_done_ms` shows whether Twilio echoed the mark (< 3000) or we timed out; if the line hangs a beat too long after the goodbye, lower `outcomeWaitMs` / the mark cap in `defaultCloseTiming`. If a legitimate goodbye is not detected, add the phrase to `FAREWELL_RE` in `farewell.ts` (keep word boundaries).
