...
2. Say "Hello?". The agent opens purpose-first in one sentence ("Hi, I'm calling for Brian about a cleaning...") and pauses; it does not introduce itself as an AI unless you ask ("are you a robot?" gets a plain yes). If you say nothing for `OPENAI_LIVE_GREETING_WAIT_MS`, it opens anyway, same shape (`raw.greeting_fallback=true`).
