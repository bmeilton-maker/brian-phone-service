...
4. **hangup**: Twilio hangup (`raw.hangup_at`, `raw.close_ms` = trigger -> hangup; the log line `openai_live.hangup` carries `tail_after_goodbye_ms`). The callee's call is over here.
