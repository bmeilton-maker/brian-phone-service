...
2. Say "Hello?". The agent opens with one short purpose-only sentence ("Hi — calling about...") and pauses; it does not introduce itself as an AI assistant unless you ask (then it says yes in one sentence and continues). If you say nothing for `OPENAI_LIVE_GREETING_WAIT_MS`, it opens anyway, still purpose-first (`raw.greeting_fallback=true`).
