...
| `OPENAI_LIVE_GREETING_WAIT_MS`, `OPENAI_LIVE_HANGUP_DELAY_MS`, `OPENAI_LIVE_CLEAR_ON_BARGE_IN`, `OPENAI_LIVE_STORE` | `3000`, `4000`, `true`, `false` | Greeting hold, max wait for the goodbye to start before hangup, Twilio buffer clear on a substantive barge-in, keep a recording at OpenAI. |
| `OPENAI_LIVE_FAREWELL_HANGUP`, `OPENAI_LIVE_FAREWELL_SILENCE_MS` | `true`, `4000` | Hang up on farewell intent (agent goodbye sentence; callee goodbye + agent silence), not only on `end_call`. |
